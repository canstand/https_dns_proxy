...
# ARCHES="x86_64 aarch64" HTTPS_DNS_PROXY_VERSION=latest \
#     CURL_VERSION=8.14.0 \
#     TLS_LIB=openssl CONTAINER_IMAGE=debian:latest \
#     sh static-build.sh
# script will create a container and compile https_dns_proxy.

# or compile or cross-compile in docker, run:
# docker run --network host --rm -v $(pwd):/mnt -w /mnt \
#     --name "build-https_dns_proxy-$(date +%Y%m%d-%H%M)" \
#     -e RELEASE_DIR=/mnt \
#     -e ARCHES="x86_64 aarch64" \
#     -e ENABLE_DEBUG=0 \
#     -e HTTPS_DNS_PROXY_VERSION=latest \
#     -e CURL_VERSION=8.14.0 \
#     -e TLS_LIB=openssl \
...
#     debian:latest sh static-build.sh
# Supported architectures: x86_64, aarch64, armv5, armv7, i686, riscv64, s390x,
#                          mips64, mips64el, mips, mipsel, powerpc64le, powerpc


init_env() {
    # Ensure build artifacts are stored in a persistent location within RELEASE_DIR
    export DIR=${DIR:-${RELEASE_DIR}/.build_artifacts};
    ...
    mkdir -p "${DIR}" # Ensure the source/build directory exists
    echo "Release directory: ${RELEASE_DIR}"
    echo "Host Architecture: ${ARCH_HOST}"
    echo "Architecture list: ${ARCHES}"
    echo "https_dns_proxy version: ${HTTPS_DNS_PROXY_VERSION}"
    echo "libcurl version (dependency): ${CURL_VERSION}"
    echo "libev version (dependency): ${LIBEV_VERSION}"
    ...
    echo "c-ares version (dependency): ${ARES_VERSION}"
    echo "libc: ${LIBC}"
...
install_cross_compile_debian() {
    ...
    local c_target_flags="-target ${arch_compiler}-linux-${c_lib}"
    ...
    export LD="/usr/bin/${arch_compiler}-linux-${c_lib}-ld" \
           ...
           CFLAGS="${c_target_flags} -O3" \
           LDFLAGS="--ld-path=/usr/bin/${arch_compiler}-linux-${c_lib}-ld ${LDFLAGS}" \
           CXXFLAGS="${c_target_flags} -O3"; # Also set CXXFLAGS for consistency
}
...
compile_zlib() {
    ...
    if [ -f "${PREFIX}/lib/libz.a" ]; then
        echo "Skipping zlib compilation for ${ARCH}, libz.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_libunistring() {
    ...
    if [ -f "${PREFIX}/lib/libunistring.a" ]; then
        echo "Skipping libunistring compilation for ${ARCH}, libunistring.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_libidn2() {
    ...
    if [ -f "${PREFIX}/lib/libidn2.a" ]; then
        echo "Skipping libidn2 compilation for ${ARCH}, libidn2.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_libpsl() {
    ...
    if [ -f "${PREFIX}/lib/libpsl.a" ]; then
        echo "Skipping libpsl compilation for ${ARCH}, libpsl.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_ares() {
    ...
    if ( [ -f "${PREFIX}/lib/libcares.a" ] || [ -f "${PREFIX}/lib64/libcares.a" ] ); then
        echo "Skipping c-ares compilation for ${ARCH}, libcares.a already exists in ${PREFIX}" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_tls() {
    ...
    if ([ -f "${PREFIX}/lib/libssl.a" ] || [ -f "${PREFIX}/lib64/libssl.a" ]) && \
       ([ -f "${PREFIX}/lib/libcrypto.a" ] || [ -f "${PREFIX}/lib64/libcrypto.a" ]); then
        echo "Skipping ${TLS_LIB} compilation for ${ARCH}, libraries already exist in ${PREFIX}" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_libssh2() {
    ...
    if [ -f "${PREFIX}/lib/libssh2.a" ]; then
        echo "Skipping libssh2 compilation for ${ARCH}, libssh2.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_nghttp2() {
    ...
    if [ -f "${PREFIX}/lib/libnghttp2.a" ]; then
        echo "Skipping nghttp2 compilation for ${ARCH}, libnghttp2.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_ngtcp2() {
    ...
    if [ -f "${PREFIX}/lib/libngtcp2.a" ]; then
        echo "Skipping ngtcp2 compilation for ${ARCH}, libngtcp2.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_nghttp3() {
    ...
    if [ -f "${PREFIX}/lib/libnghttp3.a" ]; then
        echo "Skipping nghttp3 compilation for ${ARCH}, libnghttp3.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_brotli() {
    ...
    # Brotli has multiple static libs, checking for one of them.
    if [ -f "${PREFIX}/lib/libbrotlidec-static.a" ] || [ -f "${PREFIX}/lib/libbrotlidec.a" ]; then # Name might vary slightly
        echo "Skipping brotli compilation for ${ARCH}, brotli libraries already exist in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_zstd() {
    ...
    if [ -f "${PREFIX}/lib/libzstd.a" ]; then
        echo "Skipping zstd compilation for ${ARCH}, libzstd.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
...
compile_libev() {
    echo "Compiling libev, Arch: ${ARCH}" | tee "${RELEASE_DIR}/running"
    if [ -f "${PREFIX}/lib/libev.a" ]; then
        echo "Skipping libev compilation for ${ARCH}, libev.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;

    # Use a fixed version or allow override via LIBEV_VERSION
    local libev_v=${LIBEV_VERSION:-4.33}
    url="https://dist.schmorp.de/libev/libev-${libev_v}.tar.gz"
    download_and_extract "${url}"

    # Add -fPIC for libraries
    ...
    export CFLAGS="${original_cflags}" # Restore CFLAGS

    _copy_license LICENSE libev;
}
...
compile_curl() {
    ...
    if [ -f "${PREFIX}/lib/libcurl.a" ]; then
        echo "Skipping curl compilation for ${ARCH}, libcurl.a already exists in ${PREFIX}/lib" | tee -a "${RELEASE_DIR}/running"
        return 0
    fi
    local url
    change_dir;
    ...
    if [ "${CURL_VERSION}" = "dev" ]; then
        ...
        # CURL_VERSION is now for the libcurl dependency, should be set via env
    fi
...
compile_https_dns_proxy() {
    echo "Compiling https_dns_proxy, Arch: ${ARCH}" | tee "${RELEASE_DIR}/running"
    local url

    # The script is expected to be run from the root of https_dns_proxy project
    # In Docker, RELEASE_DIR is the project root.
    cd "${RELEASE_DIR}" || { echo "Failed to cd to ${RELEASE_DIR}"; exit 1; }

    # Set HTTPS_DNS_PROXY_VERSION based on git commit, if not already set or set to 'latest'
    # init_env already prints the initial HTTPS_DNS_PROXY_VERSION from environment
    if [ -d .git ]; then
        if [ "${HTTPS_DNS_PROXY_VERSION}" = "latest" ] || [ -z "${HTTPS_DNS_PROXY_VERSION}" ]; then
            export HTTPS_DNS_PROXY_VERSION=$(git log -1 --format=%cd.%h --date=format:'%Y%m%d')
            echo "Set https_dns_proxy version from git: ${HTTPS_DNS_PROXY_VERSION}"
        else
            echo "Using provided https_dns_proxy version: ${HTTPS_DNS_PROXY_VERSION}"
        fi
    else
        echo "Warning: ${RELEASE_DIR} is not a git repository. Cannot determine version from git."
        if [ "${HTTPS_DNS_PROXY_VERSION}" = "latest" ] || [ -z "${HTTPS_DNS_PROXY_VERSION}" ]; then
             echo "Error: HTTPS_DNS_PROXY_VERSION is not set and cannot be determined from git. Using 'unknown'."
             export HTTPS_DNS_PROXY_VERSION="unknown-$(date +%Y%m%d)"
        fi
    fi
    echo "Final https_dns_proxy version for build: ${HTTPS_DNS_PROXY_VERSION}"

    rm -rf build # Ensure a clean CMake configuration each time
    mkdir -p build
    cd build
    ...
    cmake .. \
        ...
        -DCMAKE_EXE_LINKER_FLAGS="-static -Wl,-Bstatic ${LDFLAGS}"
    make -j "$(nproc)";
    make install;
    cd .. # Back to ${RELEASE_DIR} (project root)
    _copy_license LICENSE https_dns_proxy;
}

install_https_dns_proxy() {
    mkdir -p "${RELEASE_DIR}/release/bin/"

    ls -l "${PREFIX}/bin/https_dns_proxy";
    cp -pf "${PREFIX}/bin/https_dns_proxy" "${RELEASE_DIR}/release/bin/https_dns_proxy-linux-${ARCH}${libc_flag}";

    if [ ! -f "${RELEASE_DIR}/release/version.txt" ]; then
        echo "${HTTPS_DNS_PROXY_VERSION:-unknown}" > "${RELEASE_DIR}/release/version.txt"
    fi
    # No simple -V for https_dns_proxy, skip version-info.txt for now or adapt if needed

    if [ -n "${STATIC_LIBRARY}" ]; then
        XZ_OPT=-9 tar -Jcf "${RELEASE_DIR}/release/https_dns_proxy-linux-${ARCH}-dev-${HTTPS_DNS_PROXY_VERSION:-unknown}.tar.xz" -C "${DIR}" "curl-${ARCH}"
    fi
}
...
_build_in_docker() {
    ...
    local container_image base_name current_time

    cd "$(dirname "$0")";
    base_name=$(basename "$0")
    current_time=$(date "+%Y%m%d-%H%M")
    container_image=${CONTAINER_IMAGE:-debian:latest}
    ...
    docker run --rm \
        ...
        -e HTTPS_DNS_PROXY_VERSION="${HTTPS_DNS_PROXY_VERSION}" \
        -e CURL_VERSION="${CURL_VERSION}" \
        -e LIBEV_VERSION="${LIBEV_VERSION}" \
        -e TLS_LIB="${TLS_LIB}" \
...
compile() {
    ...
    # Dependencies for libcurl and https_dns_proxy
    compile_tls;
    compile_zlib;
    compile_zstd;
    compile_libunistring;
    ...
    # Dependency for https_dns_proxy
    compile_curl;
    compile_libev;

    compile_https_dns_proxy; # Compiles the main application
    install_https_dns_proxy; # Installs the main application
}
...
main() {
    ...
    echo "Compiling https_dns_proxy for all ARCHes: ${ARCHES}"
